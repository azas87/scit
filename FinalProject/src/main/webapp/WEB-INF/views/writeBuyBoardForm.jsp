...
<body>
	...
	어종<input type="text" id ="fishName" name="fishName"/>
		...
		산지<select id="location" name="buyLocation"></select></br>
		...
		<input type="hidden" id ="uploadDate" name="uploadDate"/></br>
		 ...
		 日付<select id="loadDate" name="birthMonth"></select>
       	 
       	 <select id="hour" name="birthDate">
		 ...
		 <select id="minute" name="birthDate">
		...
		우선순위2<select id="priority1" name="priority2">
					<c:forEach var="buy" items="${buylist}">
					  <option value="${buy.successSellerId}">${buy.successSellerId}</option>
					</c:forEach>
				</select>
				
		우선순위3<select id="priority1" name="priority3">
					<c:forEach var="buy" items="${buylist}">
					  <option value="${buy.successSellerId}">${buy.successSellerId}</option>
					</c:forEach>
				</select>
		코멘트<input type="text" id ="buyListComment" name="buyListComment"/>
		<input type="hidden" id ="fishNum" name="fishNum"/>
		
	 <button id="btn2" onclick="return check()">완료</button>

	</form>
	
		<input type="hidden" id="setFishName" value="${fishName}">
		<input type="hidden" id="setFishNum" value="${fishNum}">
		<input type="hidden" id="setLoction" value="${buyLocation}">
		<input type="hidden" id="setSellerId" value="${sellerId}">
